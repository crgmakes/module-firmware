/**
 * @brief Class representing a relay module
 * @file SwitchModule.cpp
 * @date 26 Jan 2026
 * @author cyberreefguru
 */

#include "SwitchModule.h"

SwitchModule::SwitchModule()
{
}

SwitchModule::~SwitchModule()
{
}

void SwitchModule::initialize()
{
    // Call super initialize
    AbstractModule::initialize();

    // Setup hardware
    // TODO - figure out how to do this using channel count
    switches[0].initialize(SWITCH_1_PIN);
}

/**
 * @brief Handles write commands from the host
 */
void SwitchModule::handleReceiveEvent()
{
    // NOTE - it actually uses less memory to make these local 
    //        and not use the pointer to an array
    uint8_t command = i2cBuffer[0];
    uint8_t reg = i2cBuffer[1];
    
    if (command != SEESAW_SWITCH_BASE)
    {
        return;
    }

    if (receiveLength < 3 || receiveLength > 5)
    {
        // Not a valid command or a read/write combo
        return;
    }

    if (reg == SEESAW_SWITCH_BULK)
    {
        uint8_t value = i2cBuffer[2];
        for (uint8_t index = 0; index < MODULE_CHANNELS; index++)
        {
            switches[index].set(((value >> index) & 0x1));
        }
    }
    else if (reg == SEESAW_SWITCH_CHANNEL)
    {
        uint8_t channel = i2cBuffer[2];
        uint8_t value = i2cBuffer[3];
        if( channel >= MODULE_CHANNELS)
        {
            return;
        }
        switches[channel].set( value );
    }
}

void SwitchModule::handleRequestEvent()
{
    uint8_t command = i2cBuffer[0];
    uint8_t reg = i2cBuffer[1];
    if (command != SEESAW_SWITCH_BASE)
    {
        return;
    }

    if (reg == SEESAW_SWITCH_BULK)
    {
        uint8_t temp = 0;
        for (uint8_t index = 0; index < MODULE_CHANNELS; index++)
        {
            temp |= ( (switches[index].get() & 0x01) << index);
        }
        Wire.write(temp);
    }
    else if (reg == SEESAW_SWITCH_CHANNEL)
    {
        uint8_t channel = i2cBuffer[2];
        if( channel >= MODULE_CHANNELS)
        {
            return;
        }
        Wire.write( switches[channel].get() & 0x01 );
    }
}

Switch::Switch()
{
}

void Switch::initialize(uint8_t pin)
{
    pinMode(pin, OUTPUT);

    // TODO - get state from flash
    digitalWrite(pin, 0);
}

void Switch::set(bool b)
{
    digitalWrite(pin, b);
}

bool Switch::get()
{
    return digitalRead(pin);
}
